...
| Status | Local CPU retry stays **BLOCKED, by explicit user choice** — not relaunched proactively. **Superseded path added this pass: `hpc/` bundle for BGU's Slurm cluster** (`hpc/setup_env.sh`, `hpc/train_pilot.sbatch`, `hpc/README.md`) — a real GPU (RTX 3090), not a memory-constrained shared desktop, so the original blocker doesn't apply there. `train_pilot.py` was made environment-portable rather than forked: CUDA is detected dynamically (a tiny real op, not just `torch.cuda.is_available()` — the exact flag that lied on the local desktop's old GPU), and checkpoint storage routes through `$SLURM_SCRATCH_DIR` when set, mirrored to home storage after every save (not only at a clean exit, since a hard OOM/timeout kill can't be trapped for that). The user runs this manually — no cluster access from this session. Not yet run; report back once it has. |
